...
export default function MyPatientsToday() {
  const {
    patients,
    currentUser,
    markVisitComplete,
  ...
  const readyForVisit = myPatients.filter(
    (p) => hasTempToday(p) && !hasVisitToday(p),
  );
  const visited = myPatients.filter((p) => hasVisitToday(p));
  const blocked = myPatients.filter(
    (p) => !hasTempToday(p) && !hasVisitToday(p),
  );
  ...
  return (
    <Layout
      ...
      subtitle={`${myPatients.length} patient${myPatients.length === 1 ? "" : "s"} assigned · ${readyForVisit.length} ready for visit`}
    >
      <div className="mb-6 rounded-lg border border-primary-200 bg-primary-50/40 px-4 py-3 flex gap-3 text-sm">
        ...
        <div className="text-primary-900">
          These are the patients assigned to you for today's shift. Patients
          become "Ready for visit" automatically once their assigned nurse
            ...
            <section className="mb-10">
              ...
              <div className="card bg-amber-50/50 border-amber-200 p-4 mb-3 flex gap-3 text-sm">
                ...
                <div className="text-amber-900">
                  These patients are assigned to you but have not yet had their
                  temperature recorded. The assigned nurse will be notified.
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {blocked.map((p) => (
      ...
      {/* Patient review modal — same as Doctor.jsx */}
      <Modal
        open={!!selected}
        onClose={() => {
          setSelected(null);
              ...
              <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900 flex gap-3">
                ...
                <div>
                  ...
                  <div className="mt-0.5">
                    Visits cannot be logged until {selected.assignedNurse}{" "}
                    records today's temperature.
                  </div>
                </div>
              </div>
