  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div className="card p-5">
          <div className="flex items-center justify-between mb-3">
            <div className="text-xs font-medium text-slate-500 uppercase tracking-wide">Current occupancy</div>
            <BedDouble className="w-4 h-4 text-slate-400" />
          </div>
          <div className="flex items-baseline gap-1.5">
            <div className="font-display text-3xl text-slate-900">{occupancy}</div>
        ...
        <div className="card p-5">
          <div className="flex items-center justify-between mb-3">
            <div className="text-xs font-medium text-slate-500 uppercase tracking-wide">Pending discharges</div>
            <DoorOpen className="w-4 h-4 text-slate-400" />
          </div>
          <div className="font-display text-3xl text-slate-900">{flagged.length}</div>
          <div className="text-xs text-slate-500 mt-1">Flagged by doctors</div>
        </div>

        <div className="card p-5">
          <div className="flex items-center justify-between mb-3">
            <div className="text-xs font-medium text-slate-500 uppercase tracking-wide">Admitted today</div>
      ...
      {occupancyPct >= 95 && (
        <div className="mb-6 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-900 flex gap-3">
          ...
          <div>
            ...
            <div className="mt-0.5">Only {BED_CAPACITY - occupancy} bed(s) left. Prioritize discharging eligible patients.</div>
          </div>
        </div>
      )}
      ...
      <section className="mb-10">
        ...
        {flagged.length === 0 ? (
          <div className="card p-6 text-sm text-slate-500">No patients flagged for discharge yet.</div>
        ) : (
              ...
              <tbody className="divide-y divide-slate-100">
                {flagged.map((p) => (
                  <tr key={p.id} className="text-sm">
                    ...
                    <td className="px-5 py-4">
                      <div className="text-slate-700">{p.dischargeFlaggedBy}</div>
                      <div className="text-xs text-slate-500">{formatDate(p.dischargeFlaggedOn)}</div>
                    </td>
                    <td className="px-5 py-4 text-slate-700">
                      {daysBetween(p.admittedOn, todayStr())} days
                    </td>
                    <td className="px-5 py-4 text-right">
                      <button className="btn-primary" onClick={() => handleDischarge(p)}>
            ...
            <tbody className="divide-y divide-slate-100">
              {admitted.map((p) => (
                <tr key={p.id} className="text-sm hover:bg-slate-50">
                  ...
                  <td className="px-5 py-4 text-slate-700">{formatDate(p.admittedOn)}</td>
                  <td className="px-5 py-4 text-slate-700">{daysBetween(p.admittedOn, todayStr())} days</td>
                </tr>
              ))}
            </tbody>
          </table>
