import { Thermometer, Stethoscope, CheckCircle2, Clock, AlertTriangle } from 'lucide-react';
import { hasTempToday, hasVisitToday, consecutiveFeverFreeDays, isDischargeEligible, isFever, formatDate } from '../lib/clinical';
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-4">
        ...
        {latest && (
          <div className={`flex items-baseline gap-0.5 ${isFever(latest.value) ? 'text-critical' : 'text-success'}`}>
            <span className="font-display text-2xl leading-none">{latest.value}</span>
      ...
      <div className="flex items-center flex-wrap gap-2 mt-4">
        {/* Today's temp status */}
        <span className={`badge ${hasTempToday(patient) ? 'bg-green-50 text-success' : 'bg-amber-50 text-warning'}`}>
          <Thermometer className="w-3 h-3" />
          {hasTempToday(patient) ? 'Temp logged today' : 'Temp pending'}
        </span>

        {/* Today's visit status */}
        <span className={`badge ${hasVisitToday(patient) ? 'bg-green-50 text-success' : 'bg-slate-100 text-slate-600'}`}>
